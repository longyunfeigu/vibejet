...
export async function loginWithOAuth(provider: string, code: string): Promise<TokenPair> {
  const res = await apiClient.post<ApiEnvelope<TokenPairResponse>>(`/auth/oauth/${provider}`, {
    code,
  })
  return mapTokenPair(res.data.data)
}
