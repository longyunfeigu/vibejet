...
2. **检测已有文档**：
   ...
   - 目录为空 → **新建模式**
3. **检测上游输入**：
   - **必需**: `docs/prd.md` → 功能需求和用户故事
   - **必需**: `docs/architecture.md` → 技术约束和模块划分
   - **推荐**: `docs/api-design.md` → 接口定义
   - **推荐**: `docs/data-model.md` → 数据结构
   - **推荐**: `docs/designs/` → UI 设计稿（按 Epic 子目录组织）
   - 必需文档缺失 → 报错终止
4. **约束提取**（自动）：
...
- [ ] [设计稿对齐（如有）] `验证: Browser 截图比对 designs/{epic-id}/{文件名}`
...
- **输入来源**: prd-generator-ears, architecture, api-design, data-model
