...
> 注：可衡量性已下沉到 AC 层（Phase 4 `验证:` 三要素 + 禁模糊词）；风险 / 失败模式由 Phase 2.6 System-Wide 6 维度承载。Epic 文档不再单列 Success Criteria / Risks / Metrics（见重要规则 §21）。
...
| **用户旅程** | PRD 用户旅程章节 / PRD §11 关键步骤 | PRD §4 功能需求 + UI 设计稿（标 ⚠️ 推断） | **❓ 必问** |
...
为每个 Epic 生成 `## 用户旅程` section，说明**客户方用户**如何实际使用系统完成业务目标。它是 WHAT 层产物，不写实现方案、不列 API 细节，也不写成 UI 点击脚本。

**抽取来源**（按优先级）：
1. PRD 显式用户旅程 / 业务流程 / 场景描述
...
按实际 Story 数自动选择结构（不询问用户）：
   ...
   **展开模式的目录结构**：
   ...
   - Epic.md 保留 `## 用户旅程`、`## System-Wide Considerations`、依赖关系和 Story 列表引用
...
注：以下大部分项 Phase 5.2 预览阶段已显示给用户。此处是写盘前最后一次自动校验，发现任意 BLOCKING 项失败则中止写盘并报告。
   ...
   - [ ] 每个 Epic 有 System-Wide Considerations section，且每条考虑都能追到某个 Story 的 Integration/Error AC（或显式标记"仅文档记录"）
...
- System-Wide Considerations: 跨界约束 / 不变量 / 失败模式，每条下放为某 Story 的 Integration/Error AC
- Story 列表: 每个 Story ~25 行
- 依赖关系: Mermaid 图
- 参考文档链接

**不写**：`Success Criteria` / `Risks and Mitigations` / `Metrics` 三个 section——epic 级完成判据由 Story AC 承载，跨界约束 / 失败模式统一进 System-Wide（见重要规则 §21）。
