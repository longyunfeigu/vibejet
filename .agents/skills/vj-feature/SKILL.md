...
2. **需求澄清**（对话式，控制在 3-5 个问题）：
   ...
   - **单屏体验轨（高频）**：当需求是新增或重做某个具体页面/Route 时，用 `ui-requirement-brief` 产出**单屏级 brief**（可选，需求清楚时可直接进入下一步），再用 `ui-page-goal-structure` / `ui-state-coverage` 的口径补页面结构与状态覆盖，结果进入 Epic 页面体验地图。
...
3. **生成验收标准**：
   ...
   UI Story 额外规则：
   - 单屏体验轨使用 `ui-page-goal-structure` 的结构：页面定位、进入原因、主任务、首屏重点、P0/P1/P2/P3 信息层级、模块顺序、设计提醒。
   - 单屏体验轨使用 `ui-state-coverage` 的结构：默认、加载、空、失败、禁用、权限不足、内容过长等状态必须能追到前端 AC 或 Assumptions。
