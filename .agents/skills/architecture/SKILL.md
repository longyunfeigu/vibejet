...
```
检测到已有架构文档：docs/architecture.md
...
1. **不要替用户做决策**：呈现选项、解释权衡、询问偏好
2. **不要跳过确认**：每个重大决策需要用户明确确认
...
```
┌─────────────────┐
│ prd-generator   │  输入：PRD 文档
│     -ears       │────────────────────┐
└─────────────────┘                    │
                                       ▼
                              ┌─────────────────┐
                              │   architecture  │  本 Skill
                              │                 │
                              └────────┬────────┘
                                       │
                              ┌────────┴────────┐
                              │  codex-review   │  后置审查（自动）
                              └────────┬────────┘
                                       │
                    ┌──────────────────┼──────────────────┬──────────────────┐
                    │                  │                  │                  │
                    ▼                  ▼                  ▼                  ▼
           ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌──────────────┐
           │ api-design  │    │ data-model  │    │epic-story   │    │task-breakdown│
           │  API 接口   │    │  数据模型   │    │ -generator  │    │   任务拆分   │
           │   ✅ 已实现  │    │   ✅ 已实现  │    │   ✅ 已实现  │    │   ⏳ 待开发  │
           └─────────────┘    └─────────────┘    └─────────────┘    └──────────────┘
```

- **输入来源**：`prd-generator-ears` 产出的 PRD 文档
- **后置审查**：`codex-review` — 文档生成后自动触发独立审查
- **输出去向**：
  - `api-design` ✅：提供 API 风格、认证方案、响应格式等约束
  - `data-model` ✅：提供数据库选型、架构风格等约束
  - `epic-story-generator` ✅：提供业务模块划分、技术选型、分层约束等信息
  - `task-breakdown` ⏳：提供模块划分、技术选型等信息（待开发）
