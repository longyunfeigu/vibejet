...
`docs/tasks/plans/{date}-epic-{N}-{slug}-plan.md`，结构见 `epic-plan.template.md`。输出采用“人工 Review 主线 + 执行 Appendix”：

- **正文供人工审批**：Reviewer Summary、目标与范围、决策与 AC 偏离、跨 Epic 契约、共享设计（ERD / 流程图）、API / Schema Delta、Unit 概览与依赖 DAG。
- **Appendix 供执行**：Triage 审计、代码复用锚点、每个 Unit 的 Files / Approach / Patterns / Test、并行波次、共享文件冲突点、风险、回滚与提交步骤。
- **单元 = 一个 Epic**，每个 Story 映射为一个 Implementation Unit。
- **`Provides / Consumes`**：跨 Epic 契约。`Consumes` 优先读取上游 plan 的 `## 3. 跨 Epic 契约 > ### Provides`，兼容旧版 `§0.5 Provides`。
...
- **task 文档（执行投影）**：Phase 5 plan 定稿后，在 `docs/tasks/work/epic-{N}-{slug}/` 生成 `_ledger.md` + 每 task 一份 7 段文档（默认 `1 Unit = 1 task`，投影自 Appendix C，UI Unit 注入 Design context 块），供 `vj-work` 直接装载执行。续作 / 重跑整目录覆盖重写。
