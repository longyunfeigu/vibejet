...
- **Screen Contract 是 UI 执行合同**：字段清单与完整度要求见 `.agents/skills/_shared/ui-planning-contract.md` §6。缺任一关键字段，不得生成 frontend-composition task；回到 Story / plan 修正。
...
- **屏型特别约束（front-of-house / operational）**：屏型判定与两类屏的必写项、禁止项按 `.agents/skills/_shared/ui-planning-contract.md` §1/§4 执行，投影进 Screen Contract；本 skill 不复述条目。
...
- **UI workflow skill input**（前端 Epic）：轨道判定与缺口触发按 `.agents/skills/_shared/ui-planning-contract.md` §2（双轨）/ §3（复杂操作流判定）执行，本 skill 不复述条目。结果只进入 Screen Contract，不生成 v0/Lovable 提示词；产品/品牌方向缺口不在 plan 里临时发明风格，标记先跑产品级 `ui-requirement-brief -> vj-design-md-matcher`。
