...
- 本 skill 负责 **HOW**，但输出分三层：**Human Plan**（给人 review 的决策、Integration Design、Unit/Task 摘要、Execution Policy）、**Task Packets**（给 `vj-work` / subagent 执行的最小上下文投影）、**Project Catalog**（API / Data / UI 的长期合同）。
- **Human Plan** 记录人工 Review 摘要、待审批决策、Integration Design（术语 / 关键流程 / ERD / 状态机 / Screen Contract，仅限跨 Unit / 跨 contract 会分歧的内容）、每个 Unit/Task 的执行摘要、Task DAG / 并行波次、Execution Policy（fast / strict + 命中触发器），以及 Catalog Sync 计划。
- **Task Packets**：Phase 5 plan 定稿后，一并生成 per-task 执行文档（`work_dir` 下 7 段文档 + `_ledger.md`）。Task 文档是 Human Plan + catalog + Story AC 的执行投影，不是第二套真相源；供 `vj-work` 默认直接装载执行，减少执行期读取完整 Human Plan。
...
vj-product-requirements → vj-architecture → api-design → data-model
        → vj-epic-story (WHAT)
        → vj-epic-plan (Human Plan + task packets + catalog delta, 本 skill)
        → vj-work / run-epic (执行)
```

**铁律**：
- 本 skill 只做计划，**不写实现代码、不跑测试**。模板里的 ERD / 时序图 / 伪代码是方向性指引，不是实现规范。
- 输出按“Human Plan + Task Packets + Catalog”分层。Human Plan 是 review 主线；Task Packets 是 AI 执行入口；Catalog 是长期合同真相源。不要把三者混成一份大上下文。
...
- **Integration Design 才是真共享设计**：§4 只写跨 Unit、跨 contract、跨执行 agent 会产生分歧的信息。术语对齐、关键流程图、ERD、状态机、Screen Contract 都保留，但只在影响多个 Unit、长期合同、失败语义、权限/事务/状态流转、或需要人拍板时写。普通 CRUD、DTO、字段映射、局部 UI binding 不加假图和假术语表。
- **高风险流程才画清**：状态机、并发、计费、权限、判分、AI 评估、事务 / 幂等等 Unit，必须在 §4 Integration Design 或 Appendix C 用业务伪代码、Mermaid state / sequence 图或不变量把状态、分支、失败策略说清。
...
- **Execution Policy 取代 Flow 标签**：不再让 reviewer 消费 Flow A/B/C。保留风险触发器，但输出为 `Execution policy: fast | strict`、命中的 strict triggers、required gates（review / migration / screenshot / catalog sync）。Appendix A 只列命中的风险、来源、Scope Challenge 与执行结果。
- **写作风格（说人话，且只花在该花的地方）**：人工 review 主面是 **§4 Integration Design**（术语 / 关键流程图 / ERD / 状态机 / Screen Contract）和 **§6 Unit/Task 摘要**。说人话的预算花在这里：图注、术语解释、字段含义用大白话，像当面指着图讲这个 epic 怎么搭。给机器看的 Appendix / task 文档保持精确即可，**不需要散文润色**。契约性内容（文件路径、AC 的 `验证:` 命令、API / Schema / UI Delta、依赖、`D-ID`、T-ID）保持精确——大白话 ≠ 含糊，不能借说人话把 AC 写松、文件清单写漏。砍掉 AI 腔：空话套话（"赋能 / 打通闭环 / 实现统一"）、形容词堆砌（"强大 / 灵活 / 优雅 / 健壮"）、学术腔、强行对仗排比。自检：每段念一遍，不像在跟人口头讲就改。
...
2. 在 `Appendix A. Execution Policy` 填**命中的风险 + required gates + 来源索引**：execution policy、strict triggers、review / migration / screenshot / catalog sync 是否 required、影响范围、关键约束来源、Scope Challenge。不要展开硬约束 / 隐含约束长清单；执行相关约束投影到 Appendix C、task docs 与 `_execution_context.md`，稳定契约留在 catalog。
...
> 所有 plan 都填：§0 审批门（目标 + 待拍板决策链接）、目标与范围、决策与 AC 偏离、Integration Design（可为 none）、Unit/Task 概览、Execution Policy、Unit 执行详情、Task DAG / 波次、Sources。
> §4 Integration Design 是人工 review 主面之一，但只写跨 Unit / 跨 contract 会分歧的信息；没有真实共享设计时明确写“无”，不要画普通 CRUD 假图。
> §5 API / Schema / UI Delta 按实际 contract delta 填；无 delta 时明确“无需同步”，不要创建空 catalog。
> Story 依赖与并行与 policy 无关：本 Epic 含 ≥2 个 Story 即填写 §6 DAG + Appendix D。优先生成可安全并行的 Task DAG，但 Unit done 仍由 Story AC / Unit Verification 闭环。

### Phase 4：结构化（填模板主体）

1. **§0 审批门**（thin）：**不是 Reviewer Summary**。只写：目标一句话、“设计与方向看 §4”、“执行与并行看 §6 / Appendix D”、需要用户拍板的决策（只列 `D-ID` 链 §2，且这些决策已内联标注到 §4 图注或 §6 task 摘要）、API / Schema / UI catalog 是否受影响（链 §5）、Execution policy（fast/strict，链 Appendix A）。**砍掉范围 / 顺序 / 下游表和 Review Checklist**——范围在 §1、顺序在 §6/D，契约在 §3 / §5，不在 §0 重复。
2. **决策与 AC 偏离**（§2）：这是决策真相源。待审批事项、AC 偏离、已确认关键决策（带 `Rejected:`）集中写在这里。
3. **跨 Epic 契约（§3）——单一真相源 = catalog，不在 plan 写 Provides 表**：
...
4. **Integration Design（§4，人工 review 主面）**：只写跨 Unit / 跨 contract / 跨执行 agent 会产生分歧的内容。包括：术语表（跨 API/Data/UI 或易混淆时）、跨 Story 数据模型 ERD（只画本 Epic 拥有 + Consumes 子集，字段内联 `约束→需求`）、核心流程时序图或状态机（有状态、失败分支、权限、事务、外部调用、跨 Story 接力时才画）、前端设计上下文（**指针 + 现状冲突 / 契约**，不复制 DESIGN.md 约束原文）、UI Surface Delta / Screen Contract（前端 Epic 必填）。**把 §2 的已定关键决策内联标注到对应图或表**，让用户“扫图 = 看见并批准方向”。没有真实共享设计时明确写“无跨 Unit / contract 分歧”，不要强行画图。
...
6. **Implementation Units + Task 摘要**（§6 + Appendix C）：**每个 Story → 一个 Unit**。§6 放人工 Review 所需的 Unit/Task 摘要：Goal / 主要交付 / Depends / Files / Approach / Risk / Verification / Task packet。Appendix C 放更完整的 Files（repo 相对路径）/ Approach / Patterns / Test scenarios / Task projection。**Test scenarios 直接链接 Story AC 里已写好的 `验证:` 命令，不重写**，仅补 AC 未覆盖的用例——按来源分两类处理：
   ...
   - Unit 内若需要多个可独立执行的阶段，优先评估是否能形成可并行 task；不能独立验证或写集高度耦合时，才写进同一 task 的 `Implementation Plan`。
7. **Story 依赖、Task DAG 与并行**（§6 + Appendix D，本 skill 重点，**≥2 Story 即必填**）：
   ...
   - 先识别 **barrier / owner tasks**：schema、DTO、shared enum、route shell、Screen Contract、shared registration、ownership policy、catalog sync 等共享输出先稳定。
   - 以缩短 critical path 为目标生成 **Task DAG / 波次**：同波次 task 写集隔离、合同稳定、有独立 done signal。不要按技术层拆；按 capability / screen / barrier 拆。
   - **共享文件冲突点**:逐一检查同波次 task 是否改同一文件(常见序列化点:`unit_of_work.py` 两处、`models/__init__.py`、`main.py`、`dto.py`、`apiClient.ts`、`routeTree.gen.ts`)。逻辑独立但改同文件的 task **标为 owner task 或序列化点**(串行或一次性合并改动)。
   - Appendix D 必须同时保留 **Unit DAG** 与 **Task DAG / 波次**。下游 task 只需等待它实际依赖的上游 contract / artifact 稳定；不必机械等待整个上游 Unit 完成。但任何 partial task 都必须回指 Unit，且 Unit done 不得早于所有 sibling tasks + Story AC / Unit Verification。
...
8. **风险、回滚与执行步骤**（Appendix E）：所有 plan 保留执行步骤；命中 strict triggers 时补 Failure Modes 表与回滚 / 撤销策略；命中缓存 / 幂等 / 事务 / 并发时补对应细节。
...
2. **自检清单**:
   ...
   - [ ] §4 Integration Design 是人工 review 主面：只写跨 Unit / contract 分歧；有设计内容就画清 ERD / 时序 / 状态 / 术语 / 边界，且 §2 已定决策已内联标注到对应图或表（扫图 = 批准方向）；无真实共享设计时明确 none
   ...
   - [ ] Appendix A 只保留 Execution Policy、命中的 strict triggers、required gates、影响范围、关键来源索引与 Scope Challenge；未复制 catalog / DESIGN.md / 代码约束长清单
...
5. **生成 / 更新 task 文档**（plan + catalog 定稿后，供 vj-work 直接消费）:
   - 时机:**在 vj-plan-review 修正之后**，确保 task 文档投影自定稿 Human Plan + 已同步 catalog。续作 / 重跑时**整目录覆盖重写**(每次写盘都重生成，不做增量 diff)——task 文档是执行投影，不是跨 Epic 稳定上下文的长期真相源，也不是新的决策真相源。
   - 落点:`task_docs.work_dir`(`docs/tasks/work/epic-{N}-{slug}/`),内含 `_ledger.md`(总账/索引) + 每个 task 一份 `T{NNN}-{slug}.md`(7 段文档)。
   - 拆 task:以 Appendix D 的 **Task DAG / 波次** 为准。默认先寻找可并行 task；拆分必须满足：合同稳定、写集隔离或 owner 明确、每 task 有局部 done signal、能缩短 critical path 或降低上下文、不会把 task done 当 Unit done。若 task 太小且无独立验证，就合并进同一 task。
   ...
   - 生成 **task 级 DAG / 波次 / 共享文件冲突表** 写入 `_ledger.md`，标明每个 task 回指哪个 Unit、是否 partial task、sibling tasks、Unit done 信号，并明确 “task done != Unit done”。下游 task 只需等待它实际依赖的上游 contract / artifact 稳定；Unit done 仍需所有 sibling tasks + Story AC / Unit Verification。
   - 生成内容:按 `task_docs.template`(`references/task-doc.template.md`)投影自 Human Plan §2/§4/§6、Appendix C/D 与 catalog：Goal/Files/Approach/Patterns/Test scenarios/Verification、plan anchors、catalog anchors、write scope、read-first source pointers、relevant constraints、non-goals、stop conditions。不重新发明 HOW;「变更叙事」段保留 `_(待执行)_` 占位,由 vj-work 按 fast/strict 记录策略回写（fast 可 Phase 4 统一回写, strict 每 task 回写；Unit 收口 task 记录 Unit Verification）。若 Appendix C 声明复用对象、高风险业务流程图 / 伪代码或禁止 fallback/mock/简化实现，必须原样投影进 task 文档的 Technical Approach / Execution note。
   ...
   - 生成 `_ledger.md`:波次计划直接来自 §6/Appendix D 的 Task DAG；必须保留 Unit DAG、Task DAG、Barrier / owner task 列表、Unit→Task 映射表、共享文件冲突表、Execution Policy。
   ...
   - 生成后自检:每个 Unit 都有对应 task 文档或 sibling task 集;每个 task 有 plan anchors / catalog anchors / write scope / verification / stop conditions;UI Unit 已注入 Design / Screen context 块;`_ledger.md` 波次与 §6/Appendix D 一致;「变更叙事」段为 `_(待执行)_` 占位。
   - 告知 work_dir 绝对路径(可点击)。
6. **Handoff**:告知 plan 路径 + task 文档 work_dir 路径,并提供下一步选项:
   - `vj-work` 执行本计划:task 文档已在 step 5 生成,vj-work **直接装载 task packets + `_ledger.md` 执行**(不重新生成),并生成 `_execution_context.md`;默认不全文读取 Human Plan，只在 task anchor / strict trigger / 歧义 / 冲突时回读对应章节。分支、worktree 与记录/提交粒度由 vj-work 的 auto/fast/strict 模式决定。
