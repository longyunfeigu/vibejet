...
- 从零启动新项目，需要完整 PRD → 用 `prd-generator-ears`
...
| PRD 同步 | prd.md 存在 且 新建 Epic → 同步 | prd.md 不存在 或 追加模式 → 跳过 |
| 路由实现 | 用户要求现在实现 → 接入 run-story | 用户只要 Story 文件 → 停止 |
| 停止升级 | 功能超出单 Epic 规模 → 引导到 architecture | 澄清后仍模糊 → 引导到 prd-generator-ears |
...
2. **扫描已有 Epic**：读取 `docs/epics/` 目录，记录已有 Epic 编号和 Story 编号上限
...
1. **读取项目现状**（自动，不问用户）：
   - 读 `docs/epics/` 了解已有 Epic 和 Story
   - 读现有代码结构了解技术栈和模块划分
   - 如果 `docs/prd.md` 存在，读取了解产品上下文
   - 如果 `docs/architecture.md` 存在，读取了解架构约束
...
4. **生成 Epic 文件**：
   ...
   **新建模式**：使用 `epic-story.template.md` 格式生成新文件，放到 `docs/epics/`。

   新建时的 `source_documents` 字段：
   - 文档存在 → 填路径
   - 文档不存在 → 不填，不伪造

   Epic 编号规则：读取 `docs/epics/` 中最大编号 +1。
...
- **功能描述过于模糊**：经过 3-5 个问题后仍无法确定核心能力和边界 → 建议用户先用 `prd-generator-ears` 做完整的产品思考
- **功能规模超出 Epic 级别**：澄清后发现需要多个 Epic + 架构变更 → 建议用户先用 `architecture` 做架构设计
- **docs/epics/ 目录不存在且无法创建** → 报告问题，不伪造目录
...
```
feature（本 skill）
  ├─ 产出 → docs/epics/ 中的 Epic/Story 文件
  ├─ 可选 → 同步 docs/prd.md
  └─ 路由 → run-story → do-story → story-verify-fix → review
```

- `prd-generator-ears`：从零写 PRD。`feature` 不替代它，只在已有 PRD 时做增量追加
