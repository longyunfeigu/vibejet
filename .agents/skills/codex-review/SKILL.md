---
name: codex-review
description: 文档/设计审查闭环。将 Claude 生成的文档（PRD、架构设计、API 设计、数据模型、技术方案）发送给 Codex 进行独立审查，Claude Code 自主判断采纳建议并修改文档。在 product-requirements、api-design、architecture、data-model 等 skill 生成文档后自动执行，用户可说"跳过审查"来跳过。也可手动调用 /codex-review。
---
...
- 任何文档生成 skill（product-requirements、api-design、architecture、data-model）完成后的**自动后置步骤**
...
- `product-requirements` → 生成 PRD 后自动触发本 skill
