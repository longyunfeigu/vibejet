...
审查对象是 `vj-epic-plan` 产出的 epic-plan（章节：§0 Reviewer Summary、§2 决策与 AC 偏离、§3 Provides/Consumes、§4 共享设计、§5 API/Schema Delta、§6 Implementation Units + Story 依赖与并行、Appendix A Triage / C 文件级清单 / D 波次与共享文件冲突 / E 风险回滚执行步骤）。

---

## coherence — 一致性

**审查视角**：plan 内部是否自洽。重点：§0 Reviewer Summary 的结论是否和正文/§2 决策一致；同一决策是否只在 §2 写一次、其他章节用 D-ID 引用而非复制出分歧；ID 引用（D-ID / R-ID / Unit 编号 / Story 编号）是否都对得上、有无悬空引用；术语表概念与正文用词是否一致。

**Blocking**（必修）：存在实质矛盾，会让执行者按错的那份做——例如 §0 说"先做 A 再做 B"而 §6 DAG 是 B 依赖 A 的反向；§2 的某决策与 Appendix C 的 Approach 冲突。
